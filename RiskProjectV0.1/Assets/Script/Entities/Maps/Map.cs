...
public class Map {
    ...
    public bool IsSpawnable(Vector2Int pos) {
        return battleField[pos.x + (pos.y * size.x)].GetSpawnable();
    }

    public List<CaseData> GetAllSpawnable() {
        List<CaseData> backer = new List<CaseData>();

        foreach (CaseData cd in battleField) {
            if (cd.GetSpawnable()) {
                backer.Add(cd);
            }
        }
    ...
    public int GetTeam(Vector2Int pos) {
        return battleField[pos.x + (pos.y * size.x)].GetTeam();
    }
