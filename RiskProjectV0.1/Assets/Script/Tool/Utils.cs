...
public class Utils {
    ...
    /// <summary>
    /// Simple class, yes, it is a kind of tupple..
    /// But, 1st time i needed it i used it instead of a KeyValuePair struct
    /// </summary>
    /// <typeparam name="K">The key</typeparam>
    /// <typeparam name="V">The Value</typeparam>
    public class KVPaires<K, V> {
        public K Key;
        public V Value;

        public KVPaires(K key, V value) {
            Key = key;
            Value = value;
        }
    }
    ...
    /*public int GetCountNotNull<K,V>(List<KVPaires<K,V>> list){
        int count = 0;
