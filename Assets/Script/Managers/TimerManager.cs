...
public class TimerManager {
    ...
    public bool Contain(TimeUp timeUp, object from = null) {
        ...
        if (from != null) {
            if (TimeBookGlobal[from].Contains(timeUp)) respond = true;
            if (TimeBookGame[from].Contains(timeUp)) respond = true;
            if (TimeBookMenu[from].Contains(timeUp)) respond = true;
        }
        else {
            foreach (KeyValuePair<object, List<TimeUp>> pair in TimeBookGlobal) {
                if (pair.Value.Contains(timeUp)) respond = true;
            }
